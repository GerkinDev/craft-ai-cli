...
def health():
    ...
    health: dict[str, dict[str, Any]] = ctx.obj.orchestrator.anonymous.get('/api/v1/health', raise_for_status=False).json() # type: ignore
    tab_items = [
        {'Name': 'Environment status', 'Status': summary['status'], 'Details': str(summary['updated_at'])},
        *[{
            'Name': key,
...
def put_on(target: str, no_wait: bool):
    ...
    environment_id = ctx.obj.orchestrator.anonymous.get('/api/v1/health', raise_for_status=False).json()['environment_id']['info']
    if target == environment_status:
        click.echo('Environment is already in desired state')
        return
    if target == 'ready':
        if environment_status.startswith('resuming-') or environment_status.startswith('creating-'):
    ...
    click.echo(f'Waiting for the environment to transition to {target}. This may take a while')
    tries = 0
