...
@click.option(
    "--profile",
    help="Profile to use",
    default=os.environ.get(PROFILE_ENV_VAR, get_default_profile()),
)
...
def cli(
    ...
    if (
        ctx.get_parameter_source("profile") == click.ParameterSource.COMMANDLINE
        and no_profile
    ):
        raise click.exceptions.UsageError(
            "Cannot use both `--profile` and `--no-profile`", ctx
        )
