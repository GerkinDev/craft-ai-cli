...
def tabulize_list(data: Sequence[Mapping[str, Any]], columns: list[str | tuple[str, str]] | dict[str, str] | None = None):
    if columns is None:
...
def tabulize(columns: list[str | tuple[str, str]] | dict[str, str], data: Sequence[Mapping[str, Any]]):
...
def _tabulize(columns: dict[str, str], data: Sequence[Mapping[str, Any]]):
    ...
    return "\n".join(
        [
            ...
            line_format.format(
                "-", *["-" * min_widths[key] for key in columns.keys()]
            ),
            *[
                line_format.format(
                    " ", *[item[key] for key in columns.keys()]
                )
